...
try {
  ...
  await expectCount(page, 17, 'initial');
  ...
  await expectCount(page, 17, 'reset after alias');
  ...
  await page.locator('#search-reset').click();
  await expectCount(page, 17, 'reset after retired');
  await page.locator('#search-tiered').selectOption('yes');
  await expectCount(page, 4, 'tiered filter');

  await page.locator('#search-reset').click();
  await expectCount(page, 17, 'reset after tiered');
  await page.locator('#search-status').selectOption('reference');
  await expectCount(page, 8, 'reference filter');

  await page.locator('#search-reset').click();
  await expectCount(page, 17, 'reset after references');
  ...
  console.log('Search page passed alias, routing, status, tier, reference, and accessibility checks.');
} catch (error) {
